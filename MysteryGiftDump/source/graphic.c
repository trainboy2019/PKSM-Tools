...
char *gamesList[] = {"X", "Y", "OR", "AS", "Sun", "Moon"};
...
void gameSelectorMenu(int n) {
	pp2d_begin_draw(GFX_TOP, GFX_LEFT);
		...
		pp2d_draw_texture(BORDER, 20 + 60 * n, 112);
		
		for (int i = 0; i < 6; i++) 
			pp2d_draw_text(16 + 60 * i + (68 - pp2d_get_text_width(gamesList[i], FONT_SIZE_18, FONT_SIZE_18)) / 2, 129, FONT_SIZE_18, FONT_SIZE_18, LIGHTBLUE, gamesList[i]);
